...
from src.features.embeddings import build_embedding_matrix, save_word2vec, train_word2vec
...
class _EncodedDataset(Dataset):
    def __init__(self, df, tokenizer):
        self.input_ids = [
            torch.tensor(tokenizer.encode(text, config.MAX_SEQUENCE_LENGTH), dtype=torch.long)
            for text in df["text"]
        ]
        self.lengths = [
...
def build_model(model_name: str, vocab_size: int, embedding_matrix=None):
    ...
    if model_name == "ann":
        return ANNClassifier(config.EMBEDDING_DIM, config.ANN_HIDDEN_DIMS, num_classes, config.DROPOUT)
    if model_name == "rnn":
        return RNNClassifier(vocab_size, config.EMBEDDING_DIM, config.HIDDEN_DIM, num_classes,
                              pretrained_embeddings=embedding_matrix)
    if model_name == "lstm":
        return LSTMClassifier(vocab_size, config.EMBEDDING_DIM, config.HIDDEN_DIM, num_classes,
                               num_layers=config.LSTM_NUM_LAYERS, bidirectional=config.LSTM_BIDIRECTIONAL,
                               pretrained_embeddings=embedding_matrix)
    if model_name == "attention":
        return AttentionClassifier(vocab_size, config.EMBEDDING_DIM, config.HIDDEN_DIM, num_classes,
                                    pretrained_embeddings=embedding_matrix)
    if model_name == "transformer":
        return TransformerEncoderClassifier(vocab_size, config.EMBEDDING_DIM,
                                             num_heads=config.TRANSFORMER_NUM_HEADS,
                                             num_layers=config.TRANSFORMER_NUM_LAYERS,
                                             ff_dim=config.TRANSFORMER_FF_DIM, num_classes=num_classes,
                                             max_len=config.MAX_SEQUENCE_LENGTH, dropout=config.DROPOUT)
...
def build_dataloaders(train_df, val_df, tokenizer, batch_size: int):
    train_loader = DataLoader(_EncodedDataset(train_df, tokenizer), batch_size=batch_size,
                               shuffle=True, collate_fn=_collate)
    val_loader = None
    if val_df is not None:
        val_loader = DataLoader(_EncodedDataset(val_df, tokenizer), batch_size=batch_size,
...
def _build_assets(train_df, val_df, cache_key):
    ...
    w2v = train_word2vec(tokenized_corpus, vector_size=config.EMBEDDING_DIM, min_count=config.MIN_TOKEN_FREQ)
    ...
    assets = dict(
        tokenizer=tokenizer, embedding_matrix=embedding_matrix,
        train_loader=train_loader, val_loader=val_loader,
        vocab_size=tokenizer.vocab_size, class_weights=class_weights,
    )
...
def train_one_model(model_name: str, train_loader, val_loader, embedding_matrix, vocab_size: int,
    ...
    embedding_matrix_tensor = torch.as_tensor(embedding_matrix, dtype=torch.float32).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loader is not None:
            val_accuracy = _evaluate(model, model_name, val_loader, embedding_matrix_tensor, device)
            val_accuracy_history.append(val_accuracy)
            if val_accuracy > best_val_accuracy:
                best_val_accuracy = val_accuracy
                best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            print(f"[{model_name}] epoch {epoch + 1}/{num_epochs}  val_acc={val_accuracy:.4f}")
        else:
            print(f"[{model_name}] epoch {epoch + 1}/{num_epochs}  (final refit, no val)")
...
def run_cross_validation(model_name: str):
    ...
    for fold_index in range(config.N_FOLDS):
        ...
        result = train_one_model(
            model_name, assets["train_loader"], assets["val_loader"], assets["embedding_matrix"],
            assets["vocab_size"], assets["class_weights"], config.NUM_EPOCHS, config.LEARNING_RATE,
        )
        fold_accuracies.append(result["best_val_accuracy"])
        print(f"-> {model_name} fold {fold_index + 1}/{config.N_FOLDS}: "
...
def refit_final(model_name: str):
    ...
    result = train_one_model(
        model_name, assets["train_loader"], None, assets["embedding_matrix"],
        assets["vocab_size"], assets["class_weights"], config.NUM_EPOCHS, config.LEARNING_RATE,
    )
...
def main():
    ...
    for model_name in models_to_run:
        ...
        print(f"-> {model_name}: refit on full CV pool -> models_saved/{model_name}.pt\n")

    print("=== CV Summary ===")
    for cv_result in cv_results:
        print(f"{cv_result['model_name']:12s}  mean_acc={cv_result['mean_accuracy']:.4f} "
