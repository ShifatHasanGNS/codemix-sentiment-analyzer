...
def build_dataloaders(train_df, val_df, tokenizer, batch_size: int):
    ...
    for df, shuffle in [(train_df, True), (val_df, False)]:
        encoded = encode_batch(tokenizer, df["text"].tolist(), config.BERT_MAX_SEQUENCE_LENGTH)
        labels = torch.tensor([config.LABELS.index(label) for label in df["label"]], dtype=torch.long)
...
def fine_tune(num_epochs: int = config.BERT_NUM_EPOCHS, learning_rate: float = 2e-5):
    ...
    train_df = cv_pool.sample(n=config.BERT_TRAIN_SUBSAMPLE_SIZE, random_state=config.RANDOM_SEED)
    val_df = cv_pool.drop(train_df.index).sample(
        n=config.BERT_VAL_SUBSAMPLE_SIZE, random_state=config.RANDOM_SEED
    )
    train_df, val_df = train_df.reset_index(drop=True), val_df.reset_index(drop=True)
    print(f"BERT fine-tuning: {len(train_df)}-row train / {len(val_df)}-row val "
    ...
    train_loader, val_loader = build_dataloaders(train_df, val_df, tokenizer, config.BERT_BATCH_SIZE)
