...
def _predict_classical(model_name: str, texts: list) -> list:
    ...
    return list(
        checkpoint["classifier"].predict(transform(checkpoint["vectorizer"], texts))
    )


def _predict_neural(
    model_name: str, texts: list, tokenizer, embedding_matrix, vocab_size: int
) -> list:
    ...
    with torch.no_grad():
        for batch_texts in _batched(texts, _PREDICT_BATCH_SIZE):
            input_ids = torch.stack(
                [
                    torch.tensor(tokenizer.encode(t, config.MAX_SEQUENCE_LENGTH))
                    for t in batch_texts
                ]
            )
            lengths = torch.tensor(
                [
                    max(1, min(len(tokenizer.tokenize(t)), config.MAX_SEQUENCE_LENGTH))
                    for t in batch_texts
                ]
            )
            logits = _forward(
    ...
    with torch.no_grad():
        for batch_texts in _batched(texts, config.BERT_BATCH_SIZE):
            encoded = encode_batch(
                tokenizer, batch_texts, config.BERT_MAX_SEQUENCE_LENGTH
            )
...
def run_evaluation(models_to_evaluate, cv_results_by_model: dict | None = None):
    ...
    if skipped:
        print(
            f"Skipping evaluation for {skipped} -- no saved checkpoint found "
    ...
    for model_name in available:
        if model_name in CLASSICAL_MODELS:
            ...
            predicted = _predict_neural(
                model_name, texts, tokenizer, embedding_matrix, vocab_size
            )
        else:
        ...
        all_model_results[model_name] = evaluate_model_by_language(
            model_name, predictions_df
        )
        misclassified_by_model[model_name] = collect_misclassified_examples(
            model_name, predictions_df
        )
        print(
            f"Evaluated {model_name}: overall accuracy = "
    ...
    write_error_report(
        {
            "misclassified": misclassified_by_model,
...
def main():
    ...
    for m in selected:
        if m not in ALL_MODELS:
            raise ValueError(
                f"Unknown model {m!r} in --models; choose from {ALL_MODELS}"
            )
    if args.skip_bert:
    ...
    if classical_selected:
        print(
            f"=== Cross-validating + refitting classical baselines: {classical_selected} ==="
        )
        from src.training.train_classical import (
            fit_final_classical,
